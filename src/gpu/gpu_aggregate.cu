...
__global__ void gpu_count_kernel(const int* mask, int n, long long* result) {
...
std::int64_t gpuCount(const int* d_in, int n) {
    ...
    GpuBuffer<long long> d_res(1);
    d_res.memsetZero();

    gpu_count_kernel<<<grid, BLK, BLK * sizeof(long long)>>>(d_in, n, d_res.data());
    CUDA_CHECK(cudaDeviceSynchronize());

    long long h;
    CUDA_CHECK(cudaMemcpy(&h, d_res.data(), sizeof(long long), cudaMemcpyDeviceToHost));
